...
public interface VideoStatsMapper extends BaseMapper<VideoStatsPo> {
    ...
    @Update("UPDATE tb_video_stats SET share_count = GREATEST(share_count + #{addNum}, 0) WHERE video_id = #{videoId}")
    int updateShareCount(@Param("videoId") Long videoId, @Param("addNum") int addNum);
